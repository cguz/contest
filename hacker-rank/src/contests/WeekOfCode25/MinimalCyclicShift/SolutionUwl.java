...
public class SolutionUwl {
	...
	long[] mulfy(long[] a)
	{
		long[] b = Arrays.copyOf(a, a.length*3);
	...
	void solve()
	{
		...
		long[] mk = new long[K+2];
		for(int i = 0, sg = 1;i <= K+1;i++, sg = -sg){
			mk[i] = C(K+1, i, mod, fif)*sg;
			if(mk[i] < 0)mk[i] += mod;
		}
		
		long[] fa = convoluteSimply(a, mk, mod, 3);
		long[] fb = convoluteSimply(b, mk, mod, 3);
		long[] fc = new long[fa.length+fb.length];
		int p = 0;
		...
		fc = Arrays.copyOf(fc, p);
		int[] z = Z(fc);
		for(int i = 2*n-(K+1);i < fc.length;i++){
			if(z[i] >= n-(K+1)){
				out.println(i-(2*n-(K+1)));
				return;
			}
		}
		out.println(-1);
...
//				dw = dw * dw % P;
	...
	public static void main(String[] args) throws Exception { new SolutionUwl().run(); }
